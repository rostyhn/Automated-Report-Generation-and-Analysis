...
# so if the instructor's name is missing, it is flagged as invalid
    ...
    for match in q_matches:
        ...
        if q_num in range(PART_1_LIMITS["start"], OVERALL_EVAL_LIMITS["end"] + 1):
            ...
            if q_num not in range(19, 23):
                print(
                    f"Skipping Q{q_num}: No average mapping defined for this question number."
                )
    return pdf_json
...
def extract_comments(pdf_text, pdf_path, expected_n=None):
    ...
    if isinstance(expected_n, int) and expected_n > 0 and len(comments) > expected_n:
        comments = comments[: expected_n - 1] + [
            " ".join(comments[expected_n - 1 :]).strip()
        ]
    return comments
...
def extract_pdf(raw_pdf_path, fi):
    ...
    try:
        ...
        instructor_first_name_match = re.search(
            rf"([A-Za-z]+)\s({fi.professor})+", pdf_text
        )
        if instructor_first_name_match:
            pdf_json["eval_info"]["instructor_first_name"] = (
                instructor_first_name_match.group(1).strip()
            )

        # Extract course number
        course_no_match = re.search(
            r"(?:Course\s*No|Course\s*Number)\s*([A-Za-z0-9\-]+)",
            pdf_text,
            re.IGNORECASE,
        )
        if course_no_match:
        ...
        pdf_json["free_response"]["comments"] = extract_comments(
            pdf_text, raw_pdf_path, expected_n
        )
...
def save_json(pdf_json, fi, parsed_base_dir):
    ...
    print(
        f"  ✅ Saved json data to {os.path.join(parsed_base_dir, course_json_filename)}"
    )
...
def run_pdf_parser(pdf_source, parsed_base_dir, overwrite_json=False):
    try:
        if not os.path.exists(pdf_source):
            ...
            print(
                f"Directory created. Please add your PDF files to '{pdf_source}' folder."
            )
        else:
...
def extract_general_info(pdf_text, pdf_json):
    q19 = re.search(
        r"19\.\s+Is this a required course.*?\n([\s\S]*?)\n\s*20\.",
        pdf_text,
        re.IGNORECASE,
    )
    if q19:
        rows = [("Yes", "yes"), ("No", "no")]
        pdf_json["general_info"]["req_course_avg"] = _parse_table_block(
            q19.group(1), rows
        )
    q20 = re.search(
        r"20\.\s+What are the average hours/week.*?\n([\s\S]*?)\n\s*21\.",
        pdf_text,
        re.IGNORECASE,
    )
    if q20:
        rows = [("16", "16hr"), ("8", "8hr"), ("4", "4hr"), ("2", "2hr"), ("1", "1hr")]
        pdf_json["general_info"]["hrs_per_wk_avg"] = _parse_table_block(
            q20.group(1), rows
        )
    q21 = re.search(
        r"21\.\s+What is your class standing\?\s*\n([\s\S]*?)\n\s*22\.",
        pdf_text,
        re.IGNORECASE,
    )
    if q21:
        ...
        pdf_json["general_info"]["class_standing_avg"] = _parse_table_block(
            q21.group(1), rows
        )
    q22 = re.search(
        r"22\.\s+What % of the class meetings have you attended\?\s*\n([\s\S]*?)\n\s*23\.",
        pdf_text,
        re.IGNORECASE,
    )
    if q22:
        ...
        pdf_json["general_info"]["attended_avg"] = _parse_table_block(
            q22.group(1), rows
        )
    return pdf_json
...
def extract_free_response(pdf_text, pdf_path, expected_n=None):
    likes = _collect_blocks_between(
        pdf_path, r"^23\.\s*What did you like most about this course\?", r"^\s*24\."
    )
    dislikes = _collect_blocks_between(
        pdf_path, r"^24\.\s*What did you like least about this course\?", r"^\s*25\."
    )

    if isinstance(expected_n, int) and expected_n > 0:
        if len(likes) > expected_n:
            likes = likes[: expected_n - 1] + [
                " ".join(likes[expected_n - 1 :]).strip()
            ]
        if len(dislikes) > expected_n:
            dislikes = dislikes[: expected_n - 1] + [
                " ".join(dislikes[expected_n - 1 :]).strip()
            ]
